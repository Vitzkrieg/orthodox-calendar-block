...
export default function Edit( { attributes, setAttributes } ) {
	...
	const { dt, hh, ll, ss, tt, pw, ph, pr, pd, ps } = attributes;
			...
			<InspectorControls>
				...
				>
					...
					<ToggleControl
						...
						checked={ hh }
						onChange={ ( sh ) =>
							setAttributes( { hh: sh ? 1 : 0 } )
						}
					/>
					<SelectControl
						label={ __(
							'Display Lives of Saints',
							'orthodox-calendar'
						) }
						value={ ll }
					...
					<ToggleControl
						...
						checked={ pr === 'yes' }
						onChange={ ( popr ) =>
							setAttributes( { pr: popr ? 'yes' : 'no' } )
						}
					/>
					<ToggleControl
						label={ __(
							'Popup window dependent',
							'orthodox-calendar'
						) }
						checked={ pd === 'yes' }
						onChange={ ( popd ) =>
							setAttributes( { pd: popd ? 'yes' : 'no' } )
						}
					/>
					<ToggleControl
						label={ __(
							'Popup window scrollbars',
							'orthodox-calendar'
						) }
						checked={ ps === 'yes' }
						onChange={ ( pops ) =>
							setAttributes( { ps: pops ? 'yes' : 'no' } )
						}
					/>
				</PanelBody>
			</InspectorControls>
